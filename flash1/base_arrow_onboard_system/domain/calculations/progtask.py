...
from .helpers import ( change_sym_chord,
...
def track_calculation_new(
    ...
    measurements['strelograph_work_chord_0185'] = horde_to_horde(
                                            measurements['strelograph_work'],                 
                                            machine_params['back_horde_plan'], 
                                            machine_params['front_horde_plan'], 
                                            measuring_step, 
                                            SYM_CHORD_0185, 
                                            SYM_CHORD_0185)
    measurements['strelograph_work_chord_10'] = change_sym_chord(measurements['strelograph_work_chord_0185'], SYM_CHORD_0185, SYM_CHORD_10)

    # TODO: Проверить расчет профиля через функцию helpers.find_real_prof(msv_rix_l, msv_pro_l, msv_pro_r)
    measurements['sagging_left_chord_0185'] = horde_to_horde(
                                            measurements['sagging_left'],
                                            machine_params['back_horde_prof'], 
                                            machine_params['front_horde_prof'], 
                                            measuring_step, 
                                            SYM_CHORD_0185, 
                                            SYM_CHORD_0185)    
    measurements['sagging_left_chord_10'] = change_sym_chord(measurements['sagging_left_chord_0185'], SYM_CHORD_0185, SYM_CHORD_10)

    measurements['sagging_right_chord_0185'] = horde_to_horde(
                                            measurements['sagging_right'],
                                            machine_params['back_horde_prof'], 
                                            machine_params['front_horde_prof'], 
                                            measuring_step, 
                                            SYM_CHORD_0185, 
                                            SYM_CHORD_0185)  
    measurements['sagging_right_chord_10'] = change_sym_chord(measurements['sagging_right_chord_0185'], SYM_CHORD_0185, SYM_CHORD_10)
    ...
    match options.restrictions['optimization_parameters'].get('calc_plan_version', 'bounded'):
            case 'split' | 'split2':
...
def machine_task_from_base_data_new(base_task: ProgramTaskBaseData) -> SteppedData:
    ...
    """
    result = {
        'plan_fact':        base_task.plan.plan_fact, 
        'plan_prj':         base_task.plan.plan_prj,
        'plan_delta':       base_task.plan.plan_delta, 
        'vozv_fact':        base_task.plan.vozv_fact, 
        'vozv_prj':         base_task.plan.vozv_prj,
        'vozv_delta':       base_task.plan.vozv_prj - base_task.plan.vozv_fact,
        'prof_fact':        base_task.prof.prof_fact,        
        'prof_fact_left':   base_task.prof.prof_fact, 
        'prof_fact_right':  base_task.measurements_processed.data.sagging_right, 
        'prof_prj':         base_task.prof.prof_prj,
        'prof_delta':       base_task.prof.prof_delta,
        'prof_delta_left':  None,
        'prof_delta_right': None,
        'rail_length':      base_task.plan.rail_length,
        'fastening_tmp':    base_task.plan.fastening_tmp,
        'ballast_need':     base_task.plan.ballast_need,
        'a_nepog_fact':     base_task.plan.a_nepog_fact,
        'psi_fact':         base_task.plan.psi_fact,
        'v_wheel_fact':     base_task.plan.v_wheel_fact,
        'a_nepog_prj':      base_task.plan.a_nepog_prj,
        'psi_prj':          base_task.plan.psi_prj,
        'v_wheel_prj':      base_task.plan.v_wheel_prj,
        'ubound_plan':      base_task.plan.ubound,
        'lbound_plan':      base_task.plan.lbound,
        'ubound_prof':      base_task.prof.ubound,
        'lbound_prof':      base_task.prof.lbound,
    }    
    result['prof_delta_left'], result['prof_delta_right'] = calc_podjemki(
                                                                urov_naturn = result['vozv_fact'], 
                                                                urov_project = result['vozv_prj'], 
