...
class PWLF(object):
    ...
    def __init__(self, x, y, ubound, lbound, add_evl, degree, f, token, count_with_weights=True,
                 ...
                 delta_str_prev=0, summ_str=0, last_evl=0, summ_sdv=0, maxiter=1000,
    ...
    def assemble_regression_matrix(self, breaks, x, y, f, calc_for_predict=False):
        ...
        var = list(np.sum(matrix_a, axis=0)) + [np.sum(y) + self.summ_str] + list(ga[-1]) + [self.Gy[-1] + self.last_evl] + list(np.sum(ga, axis=0)) + \
