...
class PWLF(object):
    ...
    def fit_with_breaks(self, breaks, f):
        ...
        A, new_y_data, D, phi = self.assemble_regression_matrix(breaks, self.x_data, self.y_data, f_without_none)
        ...
        return self.calc_reward(e, ssr / len(new_y_data), D, phi)

    # def calc_reward(self, e, mean_ssr, D, phi):
    ...
    #             return mean_ssr + 5*10**9 + 10**10*self.check_inside_bounds(e)
    #         if (self.all_beta_values[i] > self.max_str2) | (self.all_beta_values[i] < -self.max_str2):
    #             return mean_ssr + 2.5*10**9 + 10**10*self.check_inside_bounds(e)
    #     return mean_ssr + 10**10*self.check_inside_bounds(e)
    
    def calc_reward(self, e, mean_ssr, D, phi):
        add_beta = np.dot(D, self.beta) + phi
        self.all_beta_values = self.get_beta(add_beta)
        return np.sum(self.all_beta_values**2) + 10 ** 10 * self.check_inside_bounds(e)
    ...
    def predict(self, x, f, beta=None, breaks=None):
        ...
        A, _, _, _ = self.assemble_regression_matrix(self.fit_breaks, x, y=None, f=f_without_none,
    ...
    def fit_with_breaks_opt(self, var):
        ...
        A, new_y_data, D, phi = self.assemble_regression_matrix(breaks, self.x_data, self.y_data, self.f_without_none)
