...
class PlanOptimization:
    ...
    def __init__(self, pkt, recalculated_rix, lower_bound, upper_bound,
                 ...
                 degree_list, f, tol, token, add_evl):
        ...
        # TODO выполнить track_split на прямые и кривые и ставить это условие только на прямых участках, значение изменить на 50
    ...
    def f_to_minimize(self, number_of_pkt, min_radius_length, min_per_length, delta_str_to_zero=False, end_evl_to_zero=False):
        ...
        lc_lbound = min_radius_length * np.ones(len(self.degree_list))
        if self.f[0] is None:
        ...
        lc_ubound = np.inf * np.ones(len(self.degree_list))
        if int(delta_str_to_zero) + int(end_evl_to_zero) == 2:
        ...
        pwlf = PWLF(x, y, ubound=ub, lbound=lb, add_evl=add_evl, degree=self.degree_list, f=self.f, token=self.token,
                    ...
                    summ_str=self.summ_str, last_evl=self.last_evl, summ_sdv=self.summ_sdv, maxiter=10 ** 6,
        ...
        pwlf.fit(len(self.degree_list))

        breaks = pwlf.fit_breaks
        boolarr = (x >= breaks[0]) & (x <= breaks[-1])
        y_vals = pwlf.predict(breaks, self.f)
        y_pred = pwlf.predict(x[boolarr], self.f)
        ...
        result = (self.degree_list, breaks, y_vals, y_pred, sdv_full)
