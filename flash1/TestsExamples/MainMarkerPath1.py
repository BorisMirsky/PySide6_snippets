...
from PySide6.QtCharts import QScatterSeries, QLineSeries, QVXYModelMapper, QChart, QChartView
...
from PySide6.QtCore import Qt, QTimer, QObject, QDateTime, QCoreApplication
from dataclasses import dataclass
from typing import Optional, List
from enum import Enum


class RailwayMarkerPosition(Enum):
    Left = -1
    Middle = 0
    Right = 1
class RailwayMarkerContinuationMode(Enum):
    Single = 0
    Multiple = 1
...
class RailwayMarker:
    title: str
    position: RailwayMarkerPosition
    mode: RailwayMarkerContinuationMode

marker = RailwayMarker("Сортир", RailwayMarkerPosition.Right, RailwayMarkerContinuationMode.Single)


class SelectMarkerController(QWidget):
    nextMarkerSelected: Signal = Signal(RailwayMarker)
    def __init__(self, parent: Optional[QWidget] = None) ->None:
        super().__init__(parent)
        self.__markers: List[RailwayMarker] = [
            RailwayMarker('picket', RailwayMarkerPosition.Right, RailwayMarkerContinuationMode.Single),
            RailwayMarker('pereezd', RailwayMarkerPosition.Left, RailwayMarkerContinuationMode.Single),
            RailwayMarker('traffic-light', RailwayMarkerPosition.Right, RailwayMarkerContinuationMode.Multiple)
        ]

        self.__markers_select_view = QListWidget()
        for marker in self.__markers:
            marker_icon = QIcon(f'static/{marker.title}.png')
            marker_item = QListWidgetItem(marker_icon, marker.title)
            marker_item.setData(Qt.ItemDataRole.UserRole + 100, marker)
            self.__markers_select_view.addItem(marker_item)

        layout = QVBoxLayout()
        self.setLayout(layout)
        layout.addWidget(self.__markers_select_view)
...
##################################################################################################################
class Chart(QWidget):
    def __init__(self, timer: QTimer, n: int, model: any, parent: QWidget = None):
        super().__init__(parent)
        ...
        self.counter = 0
        self.minPosition = 0
        self.lineMover = MoveLineController(self.minPosition, self.vertical_model)
        ...
        self.line_model = NumpyTableModel(np.ones((0, 2)), qApp)
        ...
        self.line_series = QLineSeries(self.line_model)
        #
        self.line_mapper = QVXYModelMapper(self.line_model)
        self.line_mapper.setXColumn(1)
        self.line_mapper.setYColumn(0)
        self.line_mapper.setModel(self.line_model)
        self.line_mapper.setSeries(self.line_series)
        #
        ...
        self.series3.setBrush(QPixmap("static/traffic-light.png").scaled(50, 50).toImage())
        self.series3.setPen(QColor(Qt.transparent))
        self.series3.setMarkerSize(50.0)


    # #  По тику счётчика новая метка (если есть) вынимается из данных и ставится на график
    def update_field(self) ->None:
        self.check_not_single_artificial_object(self.value)      # на каждом тике работает функция, проверяющая есть ли протяжённый ИССО
        maxPosition = max(50, self.counter + 5)
        self.minPosition = max(0, maxPosition - 50)
        self.vertical_line_position = self.minPosition + 40                        # где будет стоять отсечка!!!
        ...
        self.line_model.appendRow([self.counter, 2])                               # линейный график, типа наш жд путь
    ...
    def add_artificial_object(self, value) ->None:
        if value == 'ПИКЕТ [справа]':
            ...
            self.model1.appendRow([self.vertical_line_position, 3])
        elif value == 'ПИКЕТ [слева]':
            self.value = None
            self.series1.setMarkerShape(QScatterSeries.MarkerShapeRectangle)
            self.series1.setMarkerSize(20.0)
            self.series1.setColor('red')
            self.model1.appendRow([self.vertical_line_position, 1])
            ...
            self.model2.appendRow([self.vertical_line_position, 3])
        elif value == 'ОПОРА контактной сети [слева]':
            self.value = None
            self.series2.setMarkerShape(QScatterSeries.MarkerShapeCircle)
            self.series2.setMarkerSize(25.0)
            self.series2.setColor('green')
            self.model2.appendRow([self.vertical_line_position, 1])
        elif value == 'СИГНАЛ (светофор справа)':
            self.value = None
            self.model3.appendRow([self.vertical_line_position, 3])
            ...
            self.series3.setPen(QColor(Qt.transparent))
            #
            self.model3.appendRow([self.vertical_line_position, 1])
        elif value == 'ТРУБА':
            self.value = None
            ...
            self.model6.appendRow([self.vertical_line_position, 3])
        elif value == 'СТРЕЛКА крестовина [влево]':
            self.value = None
            self.series6.setMarkerShape(QScatterSeries.MarkerShapePentagon)
            self.series6.setMarkerSize(25.0)
            self.series6.setColor('darkRed')
            self.model6.appendRow([self.vertical_line_position, 1])
            ...
            self.model7.appendRow([self.vertical_line_position, 3])
        elif value == 'СТРЕЛКА остряк [влево]':
            self.value = None
            self.series7.setMarkerShape(QScatterSeries.MarkerShapeCircle)
            self.series7.setMarkerSize(40.0)
            self.series7.setColor('darkYellow')
            self.model7.appendRow([self.vertical_line_position, 1])
            ...
            self.model8.appendRow([self.vertical_line_position, 3])
        elif value == 'Разное [слева]':
            self.value = None
            self.series8.setMarkerShape(QScatterSeries.MarkerShapeRectangle)
            self.series8.setMarkerSize(35.0)
            self.series8.setColor('darkCyan')
            self.model8.appendRow([self.vertical_line_position, 1])
        # not single
    ...
    def check_not_single_artificial_object(self, value) ->None:
        if value:
            if value == 'ПЕРЕЕЗД [начало]':
                ...
                self.model9.appendRow([self.vertical_line_position, 2])
                ...
                self.model10.appendRow([self.vertical_line_position, 3])
            elif value == 'ПЛАТФОРМА [начало слева]':
                self.series10.setMarkerShape(QScatterSeries.MarkerShapeTriangle)
                self.series10.setMarkerSize(25.0)
                self.series10.setColor('cyan')
                self.model10.appendRow([self.vertical_line_position, 1])
                ...
                self.model11.appendRow([self.vertical_line_position, 2])
                ...
                self.model12.appendRow([self.vertical_line_position, 2])
