...
class Chart(QWidget):
    def __init__(self, timer: QTimer(qApp), n: int, model: any, parent: QWidget = None):
        ...
        #
        #self.my_list = [self.data[i] for i in range(self.minPosition, self.maxPosition, 1)]
        #
    ...
    def update_field(self) ->None:
        self.maxPosition = max(500, self.counter + 5)
        self.minPosition = max(0, self.maxPosition - 500)
        ...
        self.chart.axisX().setRange(self.minPosition, self.maxPosition)
        self.series1.append([QPointF(self.counter, self.data[self.counter])])
        self.my_list = [self.data[i] for i in range(self.minPosition, self.maxPosition, 1)]
        self.chart.axisY().setRange(min(0, min(self.my_list)), max(0, max(self.my_list)))
        self.counter += 1
...
# # #
# data = read_csv_file('data.csv')     #get_csv_column('data.csv', 'plan_delta')
# # my_list = [data[i] for i in range(0, 50, 1)]
# # print(my_list)
# 
# # весь .csv
# def minmaxValueByIndex(index):
#     print(data.iloc[:, index].min(), data.iloc[:, index].max())
# #minmaxValueByIndex(1)
# 
# def minmaxValueByIndexOnDistance(index, start, end):
#     column = data.iloc[:, index]
#     column_range = column[start:end]
#     print(column_range.max())
#     #print(data.iloc[:, index].min(), self.__storage.iloc[:, index].max())
# minmaxValueByIndexOnDistance(3,105,540)
# 
