...
from .new_horde_to_horde import new_horde_to_horde
from .consts import (    
...
def add_sym_chords_to_measurements(measurements: pd.DataFrame, machine_params: dict, measuring_step: float = 0.185, version: int = 1) -> pd.DataFrame:
    """
        Конвертируем данные измерительной поездки measurements на симметричную хорду 0.185 для расчета
        плана и профиля, а затем на хорду 10 м для программного задания на машине.
        Данные добавляюся в эту же измерительную в виде дополнительных столбцов: strelograph_work_chord_0185,
        strelograph_work_chord_10, sagging_left_chord_0185, sagging_left_chord_10, sagging_left_chord_10,
        sagging_right_chord_0185, sagging_right_chord_10.
    """
    # Выбираем версию функции
    if version == 2:
        horde_to_horde_specific = new_horde_to_horde
    else:
        horde_to_horde_specific = horde_to_horde

    measurements['strelograph_work_chord_0185'] = horde_to_horde_specific(
                                            np.array(measurements['strelograph_work']),                 
    ...
    measurements['sagging_left_chord_0185'] = horde_to_horde_specific(
                                            np.array(measurements['sagging_left']),
    ...
    measurements['sagging_right_chord_0185'] = horde_to_horde_specific(
                                            np.array(measurements['sagging_right']),
                                            machine_params['back_horde_prof'], 
                                            machine_params['front_horde_prof'], 
                                            measuring_step, 
                                            SYM_CHORD_0185, 
