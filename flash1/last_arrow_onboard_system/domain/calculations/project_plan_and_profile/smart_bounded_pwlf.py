...
from .functions.smart_lincond_matrix import generate_lincond_matrix
import numpy as np
...
class PWLF(object):
    ...
    def __init__(self, x, y, ubound, lbound, add_evl, degree, f, token, 
                 differential_evolution_tol,
                 differential_evolution_updating,
                 count_with_weights=True,
                 delta_str_to_zero=True, end_evl_to_zero=True, summ_evl_to_zero=False,
                 delta_str_prev=0, summ_str=0, summ_str_last_point=0, last_evl=0, summ_sdv=0, maxiter=1000,
        ...
        if self.EVL_minimize:
            self.Gy = get_evl(self.y_data) - self.add_evl - (np.arange(len(self.y_data)) + 1)*self.delta_str_prev
        else:
        ...
        self.f_is_not_none = self.__check_f_is_not_none()
        self.num_cond = int(self.delta_str_to_zero) + int(self.end_evl_to_zero) + int(self.summ_evl_to_zero) + int(self.f_is_not_none) > 0.5


        self.Ax_gen = None
        self.Ay_gen = None
        if self.gen_solution & self.num_cond:
            self.Ax_gen, self.Ay_gen = generate_lincond_matrix(self.f_vector, self.degree, self.delta_str_to_zero, self.end_evl_to_zero, self.summ_evl_to_zero)
    ...
    def __check_f_is_not_none(self):
    ...
    def get_index_to_drop(self):
        ...
        condition_number = int(self.delta_str_to_zero) + int(self.end_evl_to_zero) + int(self.summ_evl_to_zero)
    ...
    def assemble_regression_matrix(self, breaks, x, y, f, calc_for_predict=False):
        ...
        if not self.num_cond:
            return ga, self.Gy, None, None

        wa = np.delete(ga, self.index_to_drop, 1)
        a_drop = ga[:, self.index_to_drop]
        
        var = list(np.sum(matrix_a, axis=0)) + [np.sum(y) + self.summ_str + self.summ_str_last_point] + list(ga[-1]) + [self.Gy[-1] + self.last_evl] + list(np.sum(ga, axis=0)) + \
              [np.sum(self.Gy) + self.summ_sdv] + list(self.fit_breaks) + list(np.zeros(matrix_a.shape[1])) + list(f)
    ...
    def fit(self, n_segments, bounds=None, **kwargs):
        ...
        if len(kwargs) == 0:
            # print(f'differential_evolution(tol={self.differential_evolution_tol}, updating={self.differential_evolution_updating})')
            res = differential_evolution(min_function, bounds, constraints=lc,
                                         strategy='best1bin', maxiter=self.maxiter,
                                         popsize=50, tol=self.differential_evolution_tol,
                                         mutation=(0.5, 1), recombination=0.7,
                                         seed=self.seed, callback=None, disp=False,
                                         polish=True, init='latinhypercube',
                                         atol=1e-4, workers=self.de_workers,
                                         updating=self.differential_evolution_updating)
        else:
