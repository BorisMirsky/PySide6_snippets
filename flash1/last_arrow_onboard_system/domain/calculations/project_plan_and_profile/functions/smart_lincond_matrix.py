...
def generate_lincond_matrix(f, degree, delta_str_to_zero=False, end_evl_to_zero=False, summ_evl_to_zero=False):
    ...
    degree_modified = ((np.array(degree) + 1) % 2).astype(int)
    for i in range(np.sum(degree_modified)):
    ...
    condition_number = int(delta_str_to_zero) + int(end_evl_to_zero) + int(summ_evl_to_zero)

    # if condition_number < 0.5:
    #     return
    ...
    if condition_number == 0:
        b_list_modified = b_list
    else:
        b_list_modified = b_list[:-condition_number]
    ...
    ax = lambdify(summ_list + end_evl_list + summ_evl_list + fit_breaks + var_list + b_list_modified + f_without_none, kf_matrix)
    ay = lambdify(summ_list + end_evl_list + summ_evl_list + fit_breaks + var_list + b_list_modified + f_without_none, sv)
