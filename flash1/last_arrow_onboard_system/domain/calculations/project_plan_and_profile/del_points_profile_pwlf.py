...
class del_PWLF(object):
    ...
    def __init__(self, x, y, max_start_of_curve, min_end_of_curve,
                 ...
                 evl_minimize=False, constr=None):
        ...
        self.min_radius = 5000
        self.min_radius_2 = 10000
        self.max_str = 1000 * (self.min_radius - (self.min_radius ** 2 - 0.185 ** 2) ** 0.5)
        self.max_str2 = 1000 * (self.min_radius_2 - (self.min_radius_2 ** 2 - 0.185 ** 2) ** 0.5)
        ...
        self.Ax_gen, self.Ay_gen = generate_lincond_matrix(self.f_vector, self.degree, self.end_evl_to_zero,
                                                           self.summ_evl_to_zero)
    ...
    def assemble_regression_matrix(self, breaks, x, y, add_evl, f, calc_for_predict=False):
        ...
        for i in range(self.n_segments):
            a_list.append(np.where((x >= self.fit_breaks[i]) & (x < self.fit_breaks[i + 1]), 1.0, 0.0))
    ...
    def fit_with_breaks(self, breaks, f):
        ...
        A, new_y_data, D, phi = self.assemble_regression_matrix(breaks[1: -1], x, y, add_evl, f_without_none)
        ...
        return self.calc_reward(e, ssr / len(new_y_data), D, phi, lb, ub, w)

    def calc_reward(self, e, mean_ssr, D, phi, lb, ub, w):
        add_beta = np.dot(D, self.beta) + phi
        self.all_beta_values = self.get_beta(add_beta)
        for i in range(len(self.all_beta_values)):
            if (self.all_beta_values[i] > self.max_str) | (self.all_beta_values[i] < -self.max_str):
                return mean_ssr + 5*10**9 + 10**10*self.check_inside_bounds(e, lb, ub, w)
            if (self.all_beta_values[i] > self.max_str2) | (self.all_beta_values[i] < -self.max_str2):
                return mean_ssr + 2.5*10**9 + 10**10*self.check_inside_bounds(e, lb, ub, w)
        return mean_ssr + 10**10*self.check_inside_bounds(e, lb, ub, w)

    def check_inside_bounds(self, e, lb, ub, w):
        if not self.count_with_weights:
            w = np.ones(len(w))
        for i in range(len(e)):
            if (e[i]/w[i] < -ub[i]) | (e[i]/w[i] > -lb[i]):
                return 1
        return 0
    ...
    def predict(self, x, f, beta=None, breaks=None):
        ...
        A, _, _, _ = self.assemble_regression_matrix(self.fit_breaks, x, y=None, add_evl=add_evl, f=f_without_none,
                                                     calc_for_predict=True)
    ...
    def fit_with_breaks_opt(self, var):
        ...
        A, new_y_data, D, phi = self.assemble_regression_matrix(var, x, y, add_evl, self.f_without_none)
