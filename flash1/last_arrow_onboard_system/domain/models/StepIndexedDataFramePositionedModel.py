...
class StepIndexedDataFramePositionedModel(AbstractPositionedTableModel):
    ...
    def rowAtPosition(self, position: LocationVector1D) ->Dict[str, float]:
        """
            Возвращает строку таблицы индекс которой соответствует положению position в метрах. 
            Значение позиции должно быть от 0 до N (длина в метрах). Если положение < 0, либо > N, то метод 
            возвращает None. 
        """
        if position.meters < 0:
            return None
        
        try:
            return self.__storage.iloc[self.__positionAsIndex(position)]
        except:  # noqa: E722
            # заходит сюда, когда индекс отсутствует в таблице (position > N)
            return None
