...
if __name__ == '__main__':    
    ...
    for name, params in sensors_config.items():
        ...
        receive_code_spinbox.setFixedWidth(StyleSpinBoxWidth)
        ...
        if params.get('projection_range', False):
            ...
            receive_value_spinbox.setFixedWidth(StyleSpinBoxWidth)
    ...
    spinboxStrelographShift.setFixedWidth(StyleSpinBoxWidth)
    ...
    adjustmentTabWidget.addTab(controls_groupbox, 'Управляющие элементы')
    adjustmentTabWidget.addTab(sensors_groupbox, 'Датчики')
    
    mainLayout = QGridLayout()
    window.setLayout(mainLayout)
    mainLayout.addWidget(adjustmentTabWidget, 0, 0)
    mainLayout.addWidget(save_config_button, 1, 0)
    
    window.setWindowTitle('Настройка датчиков')
    window.show()
    sys.exit(app.exec())
