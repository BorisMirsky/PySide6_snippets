...
class LiningProcessor(QObject):
    def __init__(self,
        ...
        auto_branching: AutoBranchingProcessor, 
        # lining timer
        timer: int = 50,        
        # QObject parent
        ...
        self.__auto_branching = auto_branching
  
    def start(self):
        self.__timer.start()
        print('LiningProcessor started')
        print(f'AutoBranching enabled: {self.__auto_branching.enabled()}')
    ...
    def __processLining(self):
        ...
        # Устанавливаем положение рабочей зоны для расчета автоматического отвода
        self.__auto_branching.setPosition(work_position)
        ...
        # Если АВТООТВОД активен, то применяем отвод
        if self.__auto_branching.active():
            prof_difference_left = self.__auto_branching.adapt_prof_left(prof_difference_left)
            prof_difference_right = self.__auto_branching.adapt_prof_right(prof_difference_right)
        ...
        if program_task_control is not None:
            ...
            # Сюда заходим, когда контрольный маятник еще не доехал до начала программного задания.
            # В поле проектного значения записываем натурное значение маятника.
            self.__vozv_project_control_unit.write(self.__pendulum_control.read())
            # В поле ошибки записываем ноль (натура - проект)
            self.__indicator_pendulum_control.write(0)
        ...
        # Если АВТООТВОД активен, то применяем отвод
        if self.__auto_branching.active():
            plan_difference = self.__auto_branching.adapt_plan(plan_difference)
