...
from PySide6.QtCharts import (QChartView, QChart, QAbstractAxis, QValueAxis, QAbstractSeries,
                              QLineSeries, QVXYModelMapper, QAreaSeries, QCategoryAxis )
from PySide6.QtCore import QAbstractItemModel, QModelIndex, QObject, Qt, QPointF, QRect
...
class DynamicLineSeries(QLineSeries):
    def __init__(self, model: QAbstractItemModel, xColumn: int, yColumn: int, name: Optional[str] = '', parent: Optional[QObject] = None, color: str | None = None) ->None:
        super().__init__(parent)
...
class AutomaticalSeriesVerticalChart(QChart):
    def __init__(self, series: List[QAbstractSeries],
        ...
        if legend_font_size:
            font_legend = QFont()
            font_legend.setPixelSize(legend_font_size)
            self.legend().setFont(font_legend)
...
class MarkersChart(QChart):
    def __init__(self, positionRange: Tuple[float, float], positionReverse: bool,
        ...
        self.y_axis.append('&gt;', 10)
        self.y_axis.append('*',20)
        self.y_axis.append('&lt;', 30)
        self.y_axis.setRange(0, 30)
        
        self.setMaximumHeight(80)
        self.y_axis.setLabelsAngle(90)
        title_font = self.y_axis.titleFont()
        ...
        if 'YAxisHideLabels' in kwargs.keys():
            self.y_axis.setLabelsVisible(False)
        self.y_axis.setLabelsFont(QFont('monospace', 8))      #font_axis_y)
        self.x_axis.setLabelsFont(font_axis_y)
        axis_brush = QBrush(QColor("white"))
        if 'title' in kwargs.keys():
            self.y_axis.setTitleText(kwargs['title'])
        ...
        if kwargs.get('is_y_zero_axe_visible', True):
                ...
                self.y_zero_series.attachAxis(self.x_axis)
        self.x_axis.setReverse(positionReverse)
        if not positionReverse:
            self.x_axis.setRange(positionRange[0], positionRange[1])
        else:
            self.x_axis.setRange(positionRange[1], positionRange[0])
        #self.y_axis.setReverse(valueReverse)
        #self.y_axis.setRange(*valueRange)
...
class HorizontalMarkersChart(HorizontalChartOrientationMixin, MarkersChart):   
    pass
