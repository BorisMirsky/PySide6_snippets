...
from domain.dto.Travelling import PicketDirection, LocationVector1D, MeasuringTripReverseType
...
from domain.calculations.helpers import ( reverse_measuring_trip )
...
class DetailedRestrictionsWidget(QWidget):
    ...
    def __init__(self,
        ...
        self.__window.setTitle("Расчёт программного задания > Детальные ограничения")
        ...
        self.__measurements_data = self.__options.measuring_trip.measurements.data.copy()

        if self.__options.measuringDetailedRestrictionsReverseType() != MeasuringTripReverseType.Nothing:
            self.__measurements_data = reverse_measuring_trip(self.__options.measuring_trip.measurements.data, options.measuringDetailedRestrictionsReverseType())
        
        self.measurements_model = ReducedStepIndexedPositionedModel(
            step=self.__options.measuring_trip.measurements.step,
            data=self.__measurements_data,
            parent=self,
        )
        self.__picket_measurements_model = PicketPositionedTableModel(self.__options.picket_direction,
                                                                      self.__options.start_picket.meters, self)
        ...
        self.model = VerticalLineModel(self.currentPosition, rangeX=rangeX, accValue=True)
        ...
        #for i in range(min(round(self.position_max), 0), max(round(self.position_max), 0)):
        for i in range(round(self.position_min), round(self.position_max)):
            self.strelograph_top_line_series.append(i, self.__restrictions['segments'][0]['shifting_left'])
            self.strelograph_bottom_line_series.append(i, self.__restrictions['segments'][0]['shifting_right'])
        ...
        for i in range(round(self.position_min), round(self.position_max)):
            self.lifting_top_line_series.append(i, self.__restrictions['segments'][0]['raising_ubound'])  # restrictions[0])
            self.lifting_bottom_line_series.append(i, self.__restrictions['segments'][0]['raising_lbound'])  # restrictions[1])
        ...
        for i in range(round(self.position_min), round(self.position_max)):
            self.speed_series_gruz.append(i, self.__restrictions['segments'][0]['v_gruz'])
            self.speed_series_pass.append(i, self.__restrictions['segments'][0]['v_pass'])
        ...
        #####################################   Markers   ############################################
        self.chart_view_situation_empty_chart = MarkersChart(title="Ситуация", 
                                                             verticalModel=self.model, 
                                                             options = self.__options, 
                                                             parent=self).getChart()
        hbox5.addWidget(self.chart_view_situation_empty_chart, 9)

        #
        #QShortcut(Qt.Key.Key_Tab, self, self.check_focus)
    ...
    # функция проверки 'кто сейчас под tab'ом'. Пока что не используется.
...
############################################### сегменты ######################################################
    ...
    # Вернёт 'k' & 'b' (y = kx + b)
    # Для скошенных углов. Временно не используется
    ...
    #####################################################################################################################
    def __createChart(self, column_name, title:str, restrictions_values_range:list = False):
        # Графики с данными, без красных ограничительных линий
        if len(column_name) != 0:
            if len(column_name) == 1:
                chart_value_range0: (float, float) = (
                    self.__measurements_data.loc[:, column_name[0]].min(),
                    self.__measurements_data.loc[:, column_name[0]].max())
                chart_value_range = (min(chart_value_range0[0], chart_value_range0[1], 0),
                                     max(chart_value_range0[0], chart_value_range0[1], 0))
                chart_value_range_length = chart_value_range[1] - chart_value_range[0]
                chart_value_min: float = chart_value_range[0] - chart_value_range_length * 0.05
                chart_value_max: float = chart_value_range[1] + chart_value_range_length * 0.05
                ...
                chart = HorizontalChart((self.position_min, self.position_max),
                                        ...
                                        x_tick=100, y_tick=10, title=title, xMinorTickCount=9,
                ...
                chart_value_range1: (float, float) = (
                    self.__measurements_data.loc[:, column_name[1]].min(),
                    self.__measurements_data.loc[:, column_name[1]].max())
                chart_value_range = (
                ...
                chart_value_range2: (float, float) = (
                    self.__measurements_data.loc[:, column_name[2]].min(),
                    self.__measurements_data.loc[:, column_name[2]].max())
                chart_value_range = (
    ...
    def eventFilter(self, watched: QObject, event: QEvent):
        ...
        if event.type() == QEvent.Type.KeyPress:
            ...
            if event.key() == Qt.Key.Key_Left:
                ...
                self.lineMapper_for_static_line.setModel(self.static_vertical_model(self.currentPosition)) # + self.counter))
    ...
    def __leftColumn(self):
        ...
        #track_title = QLabel(self.__options.measuring_trip.options.track_title)
        widget_btn = QWidget()
        ...
        #vbox_btn.addWidget(track_title)
        vbox_btn.addStretch(5)
    ...
    def __returnData(self, i: int):
        ...
        new_index = self.position_multiplyer * self.__picket_measurements_model.getIndexByPicket(self.currentPosition)    # getStepByPicket
        #new_index1 = math.fabs(int((self.currentPosition - self.__options.start_picket.meters) * self.position_multiplyer / 0.185)) #self.__options.measuring_trip.measurements.step))
        try:
            ...
            self.plan_fact_lbl.setNum(
                round(self.__measurements_data.loc[:,'strelograph_work'].tolist()[new_index], 1))
            self.lbl_profile_value1.setNum(
                round(self.__measurements_data.loc[:,'sagging_left'].tolist()[new_index], 1))
            self.lbl_profile_value2.setNum(
                round(self.__measurements_data.loc[:,'sagging_right'].tolist()[new_index], 1))
            self.level_lbl_value1.setNum(
                round(self.__measurements_data.loc[:, 'pendulum_work'].tolist()[new_index],1))
        # self.level_lbl_value2.setNum(
        #     round(self.__measurements_data.loc[:, 'pendulum_control'].tolist()[new_index],1))
        # self.level_lbl_value3.setNum(
        #     round(self.__measurements_data.loc[:, 'pendulum_front'].tolist()[new_index],1))
            self.current_position_value.setNum(round(self.currentPosition))
    ...
    def __handle_plan_btn(self):
        ...
        for btn in self.buttons:
            if btn is sender:
                btn.setStyleSheet("background-color: #A8DADC")
            else:
                btn.setStyleSheet(focus_style)
        # ==========================================================
        self.chart_view_strelograph_empty_chart.setStyleSheet("border:13px solid #A8DADC;border-left: none;border-top: none;border-bottom: none;")
        self.chart_view_profile_empty_chart.setStyleSheet("")
        self.chart_view_speed_empty_chart.setStyleSheet("")
        self.chart_view_situation_empty_chart.setStyleSheet("")
    ...
    # 50 верхняя
    def __handleLineEditLeftPlan(self):                     # , value:str
        if self.current_chart_view == "plan":
            ...
            for i in range(min(int(self.__options.start_picket.meters + self.static_line_position), int(self.currentPosition)),
                ...
                self.strelograph_top_line_series.replace(i - self.__options.start_picket.meters, i, new_y)
            self.plan_points.append([min(self.__options.start_picket.meters + self.static_line_position, self.currentPosition), self.__restrictions['segments'][0]['shifting_left'] ])
            ...
            self.plan_points.append([max(self.__options.start_picket.meters + self.static_line_position, self.currentPosition), self.__restrictions['segments'][0]['shifting_left'] ])
    ...
    # -50 нижняя
    def __handleLineEditRightPlan(self):                     #, value:str
        if self.current_chart_view == "plan":
            new_y = int(self.shifts_right_value.text())
            for i in range(min(int(self.__options.start_picket.meters + self.static_line_position), int(self.currentPosition)),
                max(int(self.__options.start_picket.meters + self.static_line_position), int(self.currentPosition)), 1):
                    self.strelograph_bottom_line_series.replace(i - self.__options.start_picket.meters, i, new_y)
    ...
    def __profileWidget(self):
        ...
        vbox_values.addWidget(self.lbl_profile_value1) #sagging_left
        vbox_values.addWidget(self.lbl_profile_value2) #sagging_right
    ...
    def __handle_profile_btn(self):
        ...
        for btn in self.buttons:
            if btn is sender:
                ...
                btn.setStyleSheet(focus_style)
        self.chart_view_strelograph_empty_chart.setStyleSheet("")
        self.chart_view_profile_empty_chart.setStyleSheet("border:13px solid #A8DADC; border-left: none; border-top: none; border-bottom: none;")
        self.chart_view_speed_empty_chart.setStyleSheet("")
        self.chart_view_situation_empty_chart.setStyleSheet("")
        self.chart_view_sagging_chart.setStyleSheet("")
    ...
    def __handleLineEditLiftingRight(self):
        if self.current_chart_view == "profile":
            ...
            #     self.__rangeError(0, 999)
            # else:
            new_y = int(self.lifting_right_value.text())
            for i in range( min(int(self.__options.start_picket.meters + self.static_line_position), int(self.currentPosition)),
                    max(int(self.__options.start_picket.meters + self.static_line_position), int(self.currentPosition)), 1):
                self.lifting_top_line_series.replace(i - self.__options.start_picket.meters, i, new_y)
    ...
    def __handleLineEditLiftingLeft(self):
        if self.current_chart_view == "profile":
            ...
            for i in range(
                        ...
                        1):
                self.lifting_bottom_line_series.replace(i - self.__options.start_picket.meters, i, new_y)
    ...
    def __handle_speed_btn(self):
        ...
        self.chart_view_speed_empty_chart.setStyleSheet(
            "border:13px solid #A8DADC; border-left: none; border-top: none; border-bottom: none;")
        self.chart_view_situation_empty_chart.setStyleSheet("")
    ...
    def __handleLineEditSpeedPass(self):               #, value:int):
        if self.current_chart_view == "speed":
            ...
            for i in range(
                        ...
                        max(int(self.__options.start_picket.meters + self.static_line_position), int(self.currentPosition)),
                        1):
                    self.speed_series_pass.replace(i - self.__options.start_picket.meters, i, new_y)
    ...
    def __handleLineEditSpeedGruz(self):                   #, value:int):
        if self.current_chart_view == "speed":
            new_y = int(self.speed_gruz_value.text())
            for i in range( min(int(self.__options.start_picket.meters + self.static_line_position), int(self.currentPosition)),
                    max(int(self.__options.start_picket.meters + self.static_line_position), int(self.currentPosition)), 1):
                self.speed_series_gruz.replace(i - self.__options.start_picket.meters, i, new_y)
