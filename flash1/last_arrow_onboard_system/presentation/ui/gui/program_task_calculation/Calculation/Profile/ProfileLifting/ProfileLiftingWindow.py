...
class ProfileLiftingWidget(QWidget):
    ...
    def __init__(self, state: ProgramTaskCalculationSuccessState,
      ...
      #  self.summary_len = len(self.summary.elements())
        ...
        self.model = VerticalLineModel(self.options.start_picket.meters, rangeX=rangeX, accValue=True)
        ...
        self.plan_model = TrackPlanProjectModel(self.__state.calculation_result().base.track_split_plan,
                                                self.__state.calculation_result().base.plan)
        self.model1 = VerticalLineModel(self.options.start_picket.meters, rangeX=rangeX, accValue=True)
        self.model2 = VerticalLineModel(self.options.start_picket.meters + self.plan_model.elements()[0].to_dict()['end'], rangeX=rangeX, accValue=True)
        #
        grid = QGridLayout()
