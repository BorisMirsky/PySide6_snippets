...
from presentation.ui.gui.common.viewes.WindowTitle import WindowTitle, Window
...
class ProgramTaskCalculationSuccessView(QWidget):
    def __init__(self, state: ProgramTaskCalculationSuccessState,
        ...
        self.__state = state
        #Окно с заголовком
        #self.__title = WindowTitle(title="Переустройство", parent=self)
        self.__window = Window(title="Переустройство", parent=self)
        #self.__window.setContentsMargins(0,0,0,0)
        #self.__window.addWidget(self.__title)
        ...
        self.__window.addLayout(self.__layout)
    ...
    def __openReconstructionView(self):
        ...
        self.__window.setTitle("Переустройство") #Главное окно 

    def __openPlanView(self):
        self.__clearCurrentView()
        self.__currentView = PlanView(self.__calculation_result, window=self.__window)
        ...
        self.__layout.addWidget(self.__currentView)
        self.__layout.setCurrentWidget(self.__currentView)
        #setWindowTitle(self, "План")
        self.__window.setTitle("План") 

    def __openLevelView(self):
        self.__clearCurrentView()
        self.__currentView = LevelView(self.__calculation_result, window=self.__window)
    ...
    def __openProfileView(self):
        self.__clearCurrentView()
        self.__currentView = ProfileView(self.__calculation_result, window=self.__window)
...
class ReconstructionMain(QWidget):
    ...
    def __init__(self, state: ProgramTaskCalculationSuccessState, parent: QWidget = None):
        ...
        self.chart6 = MarkersChart(title="Ситуация", options = self.options, parent=self).getChart()
    ...
    def rerenderPlanCharts(self, model:TrackProjectModel):
        ...
        total_steps = new_plan_model.data.shape[0]
    ...
    def __escapeView(self):
        """Выводит диалоговое окно при нажатии Escape"""
        msg_box = QMessageBox(self)
        msg_box.setWindowTitle(
            QCoreApplication.translate('Program task calculation/success/finish message box', 'Quit'))
        msg_box.setText(QCoreApplication.translate('Program task calculation/success/finish message box',
        ...
        yes_button.setFocusPolicy(Qt.FocusPolicy.ClickFocus)
        no_button.setFocusPolicy(Qt.FocusPolicy.ClickFocus)

        #фокус на кнопку "Нет" по умолчанию
        ...
        def keyPressEvent(event):
            ...
            if key in (Qt.Key.Key_Tab, Qt.Key.Key_Right):
                if msg_box.focusWidget() == yes_button:
                    no_button.setFocus()
                else:
                    yes_button.setFocus()
            elif key == Qt.Key.Key_Left:
                if msg_box.focusWidget() == no_button:
                    yes_button.setFocus()
                else:
                    no_button.setFocus()

        msg_box.keyPressEvent = keyPressEvent  # Перезаписываем обработчик клавиш
        result = msg_box.exec()
    ...
    def __leftColumnWidget(self):
        ...
        self.level_button = QPushButton("Уровень")
        self.profile_button = QPushButton("Профиль")
        # self.results_button = QPushButton("Результаты")  # ⬅ Закомментировано
        # self.settings_button = QPushButton("Установки")
        # self.print_button = QPushButton("Печать")  # ⬅ Закомментировано
        self.save_button = QPushButton("Сохранить")
        self.escape_button = QPushButton("Выход (ESC)")

        # Добавляем кнопки в список для навигации
        self.buttons = [
            self.plan_button, self.level_button, self.profile_button,
             self.save_button, self.escape_button
        ]
        self.current_index = 0  # Индекс активной кнопки

        # Логика цвета индикаторов
        self.plan_green, self.plan_yellow, self.plan_red = True, False, False
        self.level_green, self.level_yellow, self.level_red = True, False, False
        self.profile_green, self.profile_yellow, self.profile_red = True, False, False

        # Устанавливаем стиль кнопок (с выделением активной)
        focus_style = """
            QPushButton {
                padding: 10px;
        ...
        for button in self.buttons:
            button.setStyleSheet(focus_style)
            button.setFixedWidth(200)
            button.setProperty("optionsWindowPushButton", True)

        # Подключаем сигналы кнопок
        self.plan_button.clicked.connect(self.__plan_button)
        self.level_button.clicked.connect(self.__level_button)
        self.profile_button.clicked.connect(self.__profile_button)
        # self.results_button.clicked.connect(self.__results_button)  # ⬅ Закомментировано
        # self.settings_button.clicked.connect(self.__settings_button)
        # self.print_button.clicked.connect(self.__print_button)  # ⬅ Закомментировано
        self.save_button.clicked.connect(self.__saveResult)
        self.escape_button.clicked.connect(self.__escapeView)

        # Загружаем изображения для индикаторов
        self.abs_path = os.path.dirname(os.path.abspath(__file__))
        self.img1 = QLabel(self)
        self.pixmap1 = QPixmap(os.path.join(self.abs_path, 'Data/green_narrow1.png'))
        self.img1.setPixmap(self.pixmap1)
        ...
        # Создаем горизонтальные макеты для кнопок с индикаторами
        groupbox_top_hlayout1 = QHBoxLayout()
        groupbox_top_hlayout2 = QHBoxLayout()
        groupbox_top_hlayout3 = QHBoxLayout()
        ...
        QTimer.singleShot(0, lambda: self.buttons[0].setFocus())

        return vbox

    def keyPressEvent(self, keyEvent):
        """Обрабатываем нажатия клавиш для перемещения по кнопкам"""
        key = keyEvent.key()

        if key in (Qt.Key.Key_Tab, Qt.Key.Key_Down):  # Вниз / Tab
            self.current_index = (self.current_index + 1) % len(self.buttons)
            self.buttons[self.current_index].setFocus()
        elif key == Qt.Key.Key_Up:  # Вверх
            self.current_index = (self.current_index - 1) % len(self.buttons)
            self.buttons[self.current_index].setFocus()
        elif key in (Qt.Key.Key_Return, Qt.Key.Key_Enter):  # Выбор кнопки
            self.buttons[self.current_index].click()

        # Логика изменения цвета индикаторов
