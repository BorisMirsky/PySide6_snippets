...
class SerialPortUnitProvider(AbstractUnitProvider):
    ...
    def __init__(self, configuration: dict, parent: Optional[QObject] = None) ->None:
        ...
        for sensor_name, sensor_params in configuration['models']['sensors'].items():            
            self.__sensors[sensor_name] = self.create_read_unit(sensor_name, sensor_params)
    ...
    def create_read_unit(self, sensor_name: str, sensor_params: dict) -> AbstractReadUnit:
        if sensor_name == 'tick_counter':                     
            ...
            unsigned = sensor_params.get('unsigned', False)
            print(f'unsigned={unsigned}')
            if unsigned:
                origin = ComPortReadUnit(self.__controller, sensor_params['register_id'], parent=self)                
            else:
    ...
    def __create_servo_control(self, params: dict) -> ReadWriteInterpolationUnit: 
        ...
        """
        # discrete_signals
        if params['register_id'] == 11:
            return ComPortReadWriteUnit(self.__controller, params['register_id'], QApplication.instance())
