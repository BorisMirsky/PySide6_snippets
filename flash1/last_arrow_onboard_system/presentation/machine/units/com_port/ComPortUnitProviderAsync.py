...
from PySide6.QtCore import QObject, QIODevice, QThread, QCoreApplication
...
class SerialPortUnitProviderAsync(AbstractUnitProvider):
    @classmethod
    def nop(cls, v):
        return v
    ...
    def __init__(self, configuration: dict, parent: Optional[QObject] = None) ->None:
        ...
        self.__com_worker = ComPortWorker(configuration)
        self.__com_worker.start()
        time.sleep(1)
        self.__controller = self.__com_worker.controller()
        print('COM Port Controller created asyncroniusly')

        # Разрешение/запрет обмена
        enable_register = ComPortReadWriteUnit(self.__controller, 0, self)
        enable_register.write(0)
        self.__controls['enable_register'] = enable_register

        # Признак того что контроллер обменивается пакетами
        self.__controller_heartbeat = ComPortReadUnit(self.__controller, 0, self) 
        ...
        for sensor_name, sensor_params in configuration['models']['sensors'].items():            
            if sensor_name == 'tick_counter':                     
                register = ComPortReadUnit(self.__controller, sensor_params['register_id'], self)                                       
                ...
                if not sensor_params.get('disabled', False):
                    register = SignedComPortReadUnit(self.__controller, sensor_params['register_id'], self)
                ...
                register = SignedComPortReadUnit(self.__controller, sensor_params['register_id'], parent=self)
                projection = ReadInterpolationUnit(register, LinearInterpolationStrategy(
                    (sensor_params['projection_range']['min'], sensor_params['projection_range']['max']),
                    (sensor_params['value_range']['min'], sensor_params['value_range']['max'])     ), 
    ...
    def create_read_unit(self, sensor_name: str, sensor_params: dict) -> AbstractReadUnit:
        if sensor_name == 'tick_counter':                     
            register = ComPortReadUnit(self.__controller, sensor_params['register_id'], self)                                       
            return TickCounterProvider2(register, sensor_params['inverse'], register)
        elif sensor_name == 'discrete_signals':
            return ComPortReadUnit(self.__controller, sensor_params['register_id'], self)
        elif sensor_name == 'satellite':
            if not sensor_params.get('disabled', False):
                register = SignedComPortReadUnit(self.__controller, sensor_params['register_id'], self)
                projection = ReadInterpolationUnit(register, LinearInterpolationStrategy(
                ...
                projection = MemoryBufferUnit(0.0)
            return projection
        elif sensor_name == 'strelograph_work':
            origin = SignedComPortReadUnit(self.__controller, sensor_params['register_id'], parent=self)
            strelograph_unit = StrelographUnit(origin=origin, params=sensor_params, parent=origin)
            return strelograph_unit
        else:            
            register = SignedComPortReadUnit(self.__controller, sensor_params['register_id'], parent=self)
            projection = ReadInterpolationUnit(register, LinearInterpolationStrategy(
                (sensor_params['projection_range']['min'], sensor_params['projection_range']['max']),
    ...
    def __create_servo_control(self, params: dict) -> ReadWriteInterpolationUnit: 
        ...
        origin = ComPortReadWriteUnit(self.__controller, params['register_id'], self)
        ...
        if func_type == 'piecewise_linear': 
            ...
            strategy = FunctionInterpolationStrategy(self.nop, servo_func)
        else:        
...
class ComPortWorker(QThread):
    @classmethod
    def calculate_crc_function(cls, header: bytes, data: bytes) ->bytes:
        return bytes([((256 - (sum(bytes([header[1]]) + data) & 0xFF)) & 0xFF)])
    
    def __init__(self, configuration: dict, parent: Optional[QObject] = None):
        super().__init__(parent)
    ...
    def run(self):
        try:
            self.__serial = QSerialPort(self.__config['models']['connection']['port'])        
            self.__serial.setBaudRate(self.__config['models']['connection']['baud_rate'])
            if not self.__serial.open(QIODevice.OpenMode.ReadWrite):
                raise Exception(f"Unavailable com port for connection '{self.__config['models']['connection']['port']}'")
            
            poll_timer = self.__config['models']['connection'].get('poll_timer', 50)
            receive_count = self.__config['models']['connection']['receive_items_count']
            send_count = self.__config['models']['connection']['send_items_count']
    
            self.__controller = ComPortController(
                stream = self.__serial,
                receive_options = ComPortControllerStreamOptions(bytes([0x77, 0x55]), receive_count, self.calculate_crc_function),
                send_options = ComPortControllerStreamOptions(bytes([0x77, 0xE4]), send_count, self.calculate_crc_function),
