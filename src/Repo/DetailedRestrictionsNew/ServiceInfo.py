...
def get_csv_column_range(filename:str, column_name:str, start:int, stop:int):
    df = read_csv_file(filename)
    result = df.loc[:, column_name][start:stop]
    return result.values.tolist()

def lower_range(range_to_lower:list):
    result = [x-1 for x in range_to_lower]
    return result

def lower_point(point_to_lower:list):
    result = [x-1 for x in range_to_lower]
    return result


#print(lower_shift('Data/data.csv', 'plan_delta', 500))
#print(get_csv_column_range('Data/data.csv', 'plan_delta', 660, 670))
