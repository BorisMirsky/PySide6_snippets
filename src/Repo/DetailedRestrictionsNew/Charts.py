...
#         return False
    ...
    #         self.__state.calculation_result().calculated_task.data['vozv_fact'][self.currentPosition])
    #     self.prj_label_value.setNum(
    #         self.__state.calculation_result().calculated_task.data['vozv_prj'][self.currentPosition])
    #     self.anep_label_value_fact.setNum(
    #         self.__state.calculation_result().calculated_task.data['a_nepog_fact'][self.currentPosition])
    #     self.psi_label_value_fact.setNum(
    #         self.__state.calculation_result().calculated_task.data['psi_fact'][self.currentPosition])
    #     self.fv_label_value_fact.setNum(
    #         self.__state.calculation_result().calculated_task.data['v_wheel_fact'][self.options.start_picket.meters])
    #     #
    #     self.anep_label_value_prj.setNum(
    #         self.__state.calculation_result().calculated_task.data['a_nepog_prj'][self.currentPosition])
    #     self.psi_label_value_prj.setNum(
    #         self.__state.calculation_result().calculated_task.data['psi_prj'][self.currentPosition])
    ...
    def eventFilter(self, watched: QObject, event: QEvent):
        if event.type() == QEvent.Type.KeyPress:
            ...
            # Двигаем горизонтальную черту
                ...
                return True
            elif event.key() == Qt.Key.Key_W:                      # ПЕРЕДЕЛАТЬ В СТРЕЛКУ
                self.horizontal_model.shiftLine(1)
                return True
            # ЗАКРЫТИЕ ЦИКЛА
